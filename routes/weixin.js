...
router.post('/pay-callback', wxpay.useWXCallback((msg, req, res, next) => {
  ...
  new AV.Query(Order).include('user').equalTo('tradeId', out_trade_no).first({
    ...
    return order.save({
      ...
      setTimeout(() => order.sendNotice(order.link.options && decodeURIComponent(order.link.options.title), order.link.options && decodeURIComponent(order.link.options.remark), order.link.noticeJumpUrl), 12000)
    })
  }).then(() => {
    res.success()
  }).catch(error => {
